...
const Header = () => {
  ...
  return (
    <div className="relative">
      <header className={`${hidden ? "-translate-y-[150%] opacity-0" : "translate-y-0"} fixed lg:w-full w-full z-50 top-0 md:top-4 left-1/2 -translate-x-1/2 flex justify-between lg:justify-between items-center h-[9vh] max-w-360  bg-brandLightGray  ${menuOpen ? '' : 'shadow-sm'} text-black my-0 px-6 rounded-0 lg:rounded-2xl transition-all duration-500 ease-in-out`}>
        ...
        <div className="px-4 flex items-center space-x-6">
          <div className={` ${menuOpen ? 'hidden' : ''} border-5 rounded-full border-r border-t border-black bg-brandYellow px-5 lg:px-10 py-2 lg:py-3 hover:bg-brandPurple hover:text-white transition-all duration-50 ease-in-out`}>
            <Link href="/contact" className='text-lg lg:text-xl font-bold'>Contact Us</Link>
          </div>
          <div
